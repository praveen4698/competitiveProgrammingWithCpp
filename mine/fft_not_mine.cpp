...
long long solve(vector<int> &row, vector<int> &col, vector<int> &diag) {
	...
	vector<long long> unmark_row, unmark_col;
	for(int i = 0; i < N; ++i) {
		unmark_row.push_back(markr[i] ^ 1);
		unmark_col.push_back(markc[i] ^ 1);
	}
	vector<long long> get = fft.multiply(unmark_row, unmark_col);
	for(int i = 0; i < diag.size(); ++i) {
		ans += get[diag[i]];
	}
	// vector<long long> res(2 * N, 0);
	// for(int i = 0; i < unmark_row.size(); ++i) {
	// 	for(int j = 0; j < unmark_col.size(); ++j) {
	// 		res[i + j] += unmark_row[i] * unmark_col[j];
	// 	}
	// }
